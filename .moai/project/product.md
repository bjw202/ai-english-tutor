...
### 0. 실시간 스트리밍 (Real-time Token Streaming)
모든 3개 튜터 에이전트(독해, 문법, 어휘)가 동시에 토큰 단위로 실시간 스트리밍하여 ChatGPT 스타일의 타이핑 효과를 제공합니다. SSE(Server-Sent Events)를 통해 구현되며, 각 에이전트의 토큰이 도착 순서대로 클라이언트에 전달됩니다. SPEC-VOCAB-003에서 읽기, 문법 에이전트를 배치 모드에서 스트리밍 모드로 전환하여 구현되었습니다.
